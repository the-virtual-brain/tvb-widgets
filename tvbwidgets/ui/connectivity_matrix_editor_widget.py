...
class ConnectivityMatrixEditor(TVBWidget):
    ...
    def _prepare_matrix(self, matrix_name):
        ...
        with canvas.hold_canvas(matrix_full):
            ...
            colors = self._generate_color(value=matrix[self.from_row : self.from_row + self.num_rows, self.from_col : self.from_col + self.num_cols],  matrix_name=matrix_name)
    ...
    def on_apply_change(self, change):
        ...
        with canvas.hold_canvas(matrix_ui[0]):
            matrix_ui[0].fill_style = self._generate_color(self.row, self.col, self.clicked_matrix, value)
        ...
        matrix[int(self.row)][int(self.col)] = value
        matrix_original = getattr(self.connectivity, matrix_name)

        if value > matrix_original.max() or value < matrix_original.min():
            temp = self.connectivity
            self.connectivity = self.new_connectivity
            self._update_matrices_view()
            self.connectivity = temp
